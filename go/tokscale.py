#!/usr/bin/env python3
"""Print local AI CLI token usage by model."""

from __future__ import annotations

import argparse
import json
import os
import re
import sqlite3
import sys
from collections import Counter
from datetime import datetime, timedelta, timezone
from itertools import chain
from pathlib import Path
from zoneinfo import ZoneInfo


USER_HOME = Path.home()
CODEX_DB = USER_HOME / ".codex" / "state_5.sqlite"
CODEX_DIRS = (
    USER_HOME / ".codex" / "sessions",
    USER_HOME / ".codex" / "archived_sessions",
)
CLAUDE_PROJECTS = USER_HOME / ".claude" / "projects"
PI_SESSIONS = USER_HOME / ".pi" / "agent" / "sessions"
ANTIGRAVITY_USAGE_LOG = USER_HOME / ".gemini" / "antigravity-cli" / "tokscale-usage.jsonl"
LOCAL_TZ = ZoneInfo(os.environ.get("TOKSCALE_TZ", "Europe/Oslo"))

# USD per million tokens. Claude cache writes use the 5-minute rate because
# local transcripts do not record the cache TTL.
TOKEN_FIELDS = (
    "input_tokens",
    "output_tokens",
    "cache_read_tokens",
    "cache_write_tokens",
)
CLAUDE_RATES = {
    "claude-fable-5": (10.0, 50.0, 1.0, 12.5),
    "claude-opus-5": (5.0, 25.0, 0.5, 6.25),
    "claude-sonnet-5": (2.0, 10.0, 0.2, 2.5),
}

# API-equivalent USD per million total tokens used by this report.
OPENAI_RATES = {
    "gpt-5.6-sol": 0.8,
    "codex-auto-review": 0.459224,
    "gpt-5.6-luna": 0.0177545,
}

# API-equivalent proxy rates for Antigravity's Gemini models, USD per million
# tokens. Gemini 3.6/3.7 introductory standard pricing applies through 2026.
GEMINI_PROXY_RATES = {
    "gemini-3.7-flash": (0.75, 3.75, 0.075),
    "gemini-3.6-flash": (0.75, 3.75, 0.075),
    "gemini-3.5-flash": (1.5, 9.0, 0.15),
}

PROVIDER_PREFIXES = {
    "antigravity": "agy",
    "openrouter": "or",
}


def positive_int(value) -> int:
    try:
        return max(0, int(value or 0))
    except (TypeError, ValueError):
        return 0


def slug(value: str) -> str:
    return "".join(
        "-" if character.isspace() or character == "/" else character.lower()
        for character in value
    )


def display_label(provider: str, model: str) -> str:
    prefix = PROVIDER_PREFIXES.get(provider)
    if prefix is None and provider not in ("codex", "claude"):
        prefix = slug(provider)
    model = slug(model)
    return f"{prefix}-{model}" if prefix else model


def counter_delta(current: int, previous: int) -> int:
    """Return growth since the prior snapshot, including a reset baseline."""
    current = positive_int(current)
    return current if current < previous else current - previous


def local_time(value) -> datetime:
    if isinstance(value, (int, float)):
        seconds = value / 1000 if value > 10_000_000_000 else value
        return datetime.fromtimestamp(seconds, tz=timezone.utc).astimezone(LOCAL_TZ)
    try:
        parsed = datetime.fromisoformat(str(value or "").replace("Z", "+00:00"))
    except ValueError:
        return datetime.now(LOCAL_TZ)
    if parsed.tzinfo is None:
        parsed = parsed.replace(tzinfo=timezone.utc)
    return parsed.astimezone(LOCAL_TZ)


def jsonl_records(paths):
    for path in paths:
        try:
            with path.open("r", encoding="utf-8", errors="replace") as stream:
                for line in stream:
                    try:
                        yield json.loads(line)
                    except json.JSONDecodeError:
                        continue
        except OSError:
            continue


def usage_event(
    model,
    timestamp,
    *,
    provider=None,
    input_tokens=0,
    output_tokens=0,
    cache_read=0,
    cache_write=0,
    total=None,
    cost_usd=None,
) -> dict:
    input_tokens = positive_int(input_tokens)
    output_tokens = positive_int(output_tokens)
    cache_read = positive_int(cache_read)
    cache_write = positive_int(cache_write)
    calculated_total = input_tokens + output_tokens + cache_read + cache_write
    return {
        "model": model or "unknown",
        "provider": provider or "unknown",
        "timestamp": local_time(timestamp),
        "input_tokens": input_tokens,
        "output_tokens": output_tokens,
        "cache_read_tokens": cache_read,
        "cache_write_tokens": cache_write,
        "tokens": positive_int(total) if total is not None else calculated_total,
        "cost_usd": float(cost_usd) if cost_usd is not None else None,
    }


def scan_codex_rollouts():
    """Read per-turn usage; cached input is part of input, not extra tokens."""
    for base in CODEX_DIRS:
        model = None
        for record in jsonl_records(base.rglob("*.jsonl")):
            payload = record.get("payload") or {}
            if record.get("type") == "turn_context" and payload.get("model"):
                model = payload["model"]
            usage = (payload.get("info") or {}).get("last_token_usage")
            if not model or not usage:
                continue
            yield usage_event(
                model,
                record.get("timestamp"),
                provider="codex",
                input_tokens=usage.get("input_tokens"),
                output_tokens=usage.get("output_tokens"),
                cache_read=usage.get("cached_input_tokens"),
                cache_write=usage.get("cache_write_input_tokens"),
                total=usage.get("total_tokens"),
            )


def scan_codex():
    rollouts = iter(scan_codex_rollouts())
    first = next(rollouts, None)
    if first is not None:
        yield first
        yield from rollouts
        return
    if not CODEX_DB.exists():
        return

    database = None
    try:
        database = sqlite3.connect(f"file:{CODEX_DB}?mode=ro", uri=True)
        database.row_factory = sqlite3.Row
        for row in database.execute(
            "SELECT model, updated_at, tokens_used "
            "FROM threads WHERE tokens_used > 0"
        ):
            yield usage_event(
                row["model"], row["updated_at"], provider="codex", total=row["tokens_used"]
            )
    except sqlite3.Error as error:
        print(f"warning: could not read Codex database: {error}", file=sys.stderr)
    finally:
        if database is not None:
            database.close()


def scan_claude():
    for record in jsonl_records(CLAUDE_PROJECTS.rglob("*.jsonl")):
        if record.get("type") != "assistant":
            continue
        message = record.get("message") or {}
        usage = message.get("usage") or {}
        model = message.get("model")
        if not usage or not model or model == "<synthetic>":
            continue
        yield usage_event(
            model,
            record.get("timestamp"),
            provider="claude",
            input_tokens=usage.get("input_tokens"),
            output_tokens=usage.get("output_tokens"),
            cache_read=usage.get("cache_read_input_tokens"),
            cache_write=usage.get("cache_creation_input_tokens"),
        )


def scan_pi():
    """Read Pi assistant responses, including provider and OpenRouter cost."""
    for record in jsonl_records(PI_SESSIONS.rglob("*.jsonl")):
        message = record.get("message") or {}
        usage = message.get("usage") or {}
        if record.get("type") != "message" or message.get("role") != "assistant":
            continue
        model = message.get("model")
        if not model or not usage:
            continue
        yield usage_event(
            model,
            record.get("timestamp"),
            provider=message.get("provider") or "pi",
            input_tokens=usage.get("input"),
            output_tokens=usage.get("output"),
            cache_read=usage.get("cacheRead"),
            cache_write=usage.get("cacheWrite"),
            total=usage.get("totalTokens"),
            cost_usd=(usage.get("cost") or {}).get("total"),
        )


def scan_antigravity():
    """Read opt-in status-line snapshots and emit cumulative deltas per turn."""
    snapshots = {}
    for record in jsonl_records((ANTIGRAVITY_USAGE_LOG,)):
        payload = record.get("payload") or record
        context = payload.get("context_window") or {}
        conversation = (
            payload.get("conversation_id")
            or payload.get("session_id")
            or payload.get("transcript_path")
        )
        model_info = payload.get("model") or {}
        model = model_info.get("id") or model_info.get("display_name")
        timestamp = record.get("captured_at") or payload.get("created_at")
        if not conversation or not model or not timestamp:
            continue
        totals = (
            positive_int(context.get("total_input_tokens")),
            positive_int(context.get("total_output_tokens")),
        )
        if not any(totals):
            continue
        previous = snapshots.get(conversation, (0, 0))
        snapshots[conversation] = totals
        delta_input, delta_output = map(counter_delta, totals, previous)
        if delta_input or delta_output:
            yield usage_event(
                model,
                timestamp,
                provider="antigravity",
                input_tokens=delta_input,
                output_tokens=delta_output,
                total=delta_input + delta_output,
            )


def event_cost(event: dict) -> float | None:
    if event.get("cost_usd") is not None:
        return max(0.0, float(event["cost_usd"]))
    model = event["model"]
    if event.get("provider") == "antigravity":
        normalized = model.lower().replace(" ", "-")
        for name, rates in GEMINI_PROXY_RATES.items():
            if name in normalized:
                return (
                    event["input_tokens"] * rates[0]
                    + event["output_tokens"] * rates[1]
                    + event["cache_read_tokens"] * rates[2]
                ) / 1_000_000
    if model in CLAUDE_RATES:
        return sum(
            event[field] * rate
            for field, rate in zip(TOKEN_FIELDS, CLAUDE_RATES[model])
        ) / 1_000_000
    if model in OPENAI_RATES:
        return event["tokens"] * OPENAI_RATES[model] / 1_000_000
    return None


def period_start(period: str) -> datetime | None:
    if period == "all":
        return None
    now = datetime.now(LOCAL_TZ)
    start = now.replace(hour=0, minute=0, second=0, microsecond=0)
    if period == "weekly":
        return start - timedelta(days=start.weekday())
    if period == "monthly":
        return start.replace(day=1)
    return start


def collect(period: str) -> tuple[list[dict], datetime | None, datetime | None]:
    since = period_start(period)
    models = {}
    first = last = None
    for event in chain(scan_codex(), scan_claude(), scan_pi(), scan_antigravity()):
        timestamp = event["timestamp"]
        if since and timestamp < since:
            continue
        first = timestamp if first is None or timestamp < first else first
        last = timestamp if last is None or timestamp > last else last
        key = (event["provider"], event["model"])
        row = models.setdefault(
            key,
            {
                "model": event["model"],
                "provider": event["provider"],
                "label": display_label(event["provider"], event["model"]),
                "tokens": 0,
                **dict.fromkeys(TOKEN_FIELDS, 0),
                "events": 0,
                "cost": 0.0,
                "cost_known": True,
            },
        )
        for field in ("tokens", *TOKEN_FIELDS):
            row[field] += event[field]
        row["events"] += 1
        cost = event_cost(event)
        if cost is None:
            row["cost_known"] = False
        else:
            row["cost"] += cost

    ordered = sorted(models.values(), key=lambda row: row["tokens"], reverse=True)
    return ordered, first, last


def format_tokens(value: int) -> str:
    if value >= 1_000_000_000:
        return f"{value / 1_000_000_000:.1f}B"
    if value >= 1_000_000:
        return f"{value / 1_000_000:.1f}M"
    if value >= 1_000:
        return f"{value / 1_000:.1f}K"
    return str(value)


def format_cost(value: float, known: bool) -> str:
    if not known:
        return "—"
    if value >= 1000:
        return f"${value / 1000:.1f}K"
    if 0 < value < 0.01:
        return f"${value:.3f}"
    return f"${value:.2f}"


def format_total_cost(value: float, complete: bool) -> str:
    if not complete and value == 0:
        return "—"
    return format_cost(value, True)


def interval(first: datetime | None, last: datetime | None) -> str:
    if not first or not last:
        return "no data"
    first_label = f"{first.strftime('%b')} {first.day}"
    last_label = f"{last.strftime('%b')} {last.day}"
    return first_label if first.date() == last.date() else f"{first_label} – {last_label}"


def print_models(breakdown: bool, period: str, limit: int | None = None) -> None:
    models, first, last = collect(period)
    models = models[:limit]
    model_width = max((len(row["label"]) for row in models), default=5)
    model_width = max(model_width, len("model"))
    print(f"tokscale · models · {period}")
    print(f"range: {interval(first, last)}")
    print()
    print(f"{'model':<{model_width}} {'tokens':>12} {'cost':>12}")
    print("-" * (model_width + 26))
    for model in models:
        cost = format_cost(model["cost"], model["cost_known"])
        print(
            f"{model['label']:<{model_width}} "
            f"{format_tokens(model['tokens']):>12} {cost:>12}"
        )
    total_tokens = sum(model["tokens"] for model in models)
    total_cost = sum(model["cost"] for model in models)
    total_cost_known = all(model["cost_known"] for model in models)
    print("-" * (model_width + 26))
    print(
        f"{'total':<{model_width}} "
        f"{format_tokens(total_tokens):>12} "
        f"{format_total_cost(total_cost, total_cost_known):>12}"
    )

    if not breakdown:
        return
    print()
    print("breakdown")
    print("---------")
    print(
        f"{'model':<{model_width}} {'input':>10} {'output':>10} "
        f"{'cache read':>12} {'cache write':>13} {'events':>8}"
    )
    print("-" * (model_width + 58))
    for model in models:
        print(
            f"{model['label']:<{model_width}} "
            f"{format_tokens(model['input_tokens']):>10} "
            f"{format_tokens(model['output_tokens']):>10} "
            f"{format_tokens(model['cache_read_tokens']):>12} "
            f"{format_tokens(model['cache_write_tokens']):>13} "
            f"{model['events']:>8}"
        )
    print()
    print("note: codex cache read is included inside input and is not added twice.")


def codex_session_metadata() -> dict:
    metadata = {}
    for record in jsonl_records((CODEX_DB.parent / "session_index.jsonl",)):
        if record.get("id"):
            metadata[record["id"]] = {"title": record.get("thread_name")}
    if CODEX_DB.exists():
        database = None
        try:
            database = sqlite3.connect(f"file:{CODEX_DB}?mode=ro", uri=True)
            columns = {row[1] for row in database.execute("PRAGMA table_info(threads)")}
            title_column = "COALESCE(NULLIF(name, ''), title)" if "name" in columns else "title"
            for session_id, title, started in database.execute(
                f"SELECT id, {title_column}, created_at FROM threads"
            ):
                metadata[session_id] = {
                    "title": title or metadata.get(session_id, {}).get("title"),
                    "started": local_time(started),
                }
        except sqlite3.Error as error:
            print(f"warning: could not read session titles: {error}", file=sys.stderr)
        finally:
            if database is not None:
                database.close()
    return metadata


def tool_calls(record: dict, provider: str):
    if provider == "codex":
        payload = record.get("payload") or {}
        if record.get("type") != "response_item" or payload.get("type") not in (
            "function_call", "custom_tool_call", "web_search_call", "local_shell_call"
        ):
            return
        name = payload.get("name") or payload["type"]
        yield payload.get("call_id") or payload.get("id"), name, False
        if name in ("exec", "functions.exec"):
            code = payload.get("input") or payload.get("arguments") or ""
            pattern = r'''"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|`(?:\\.|[^`\\])*`|//[^\n]*|/\*[\s\S]*?\*/|\btools\.(mcp__[\w]+)\s*\('''
            references = (match.group(1) for match in re.finditer(pattern, code) if match.group(1))
            for index, nested in enumerate(references):
                yield f"{payload.get('call_id')}:{index}", nested, True
    else:
        message = record.get("message") or {}
        if message.get("role", record.get("type")) != "assistant":
            return
        for block in message.get("content") or []:
            if isinstance(block, dict) and block.get("type") in ("tool_use", "toolCall"):
                yield block.get("id"), block.get("name") or "unknown", False


def scan_deep_sessions(period: str):
    since = period_start(period)
    metadata = codex_session_metadata()
    sources = [("codex", base) for base in CODEX_DIRS]
    sources += [("claude", CLAUDE_PROJECTS), ("pi", PI_SESSIONS)]
    seen_sessions = set()
    for provider, base in sources:
        for path in sorted(base.rglob("*.jsonl")):
            session_id, title, started, model = str(path), "", None, "unknown"
            row = {"provider": provider, "tokens": 0, **dict.fromkeys(TOKEN_FIELDS, 0),
                   "events": 0, "tools": Counter(), "mcp_refs": Counter(), "models": set()}
            seen_calls, seen_usage = set(), set()
            previous = {}
            modern = False
            for record in jsonl_records((path,)):
                if not isinstance(record, dict):
                    continue
                payload = record.get("payload") or {}
                timestamp = record.get("timestamp")
                if timestamp and started is None:
                    started = local_time(timestamp)
                kind = record.get("type")
                if provider == "codex" and kind == "session_meta":
                    session_id = payload.get("id") or payload.get("session_id") or session_id
                    if payload.get("timestamp"):
                        started = local_time(payload["timestamp"])
                elif provider == "pi" and kind == "session":
                    session_id = record.get("id") or session_id
                elif provider == "claude":
                    session_id = record.get("sessionId") or session_id
                if kind == "turn_context":
                    model = payload.get("model") or model
                title = record.get("customTitle") or record.get("summary") or title
                if kind == "session_info":
                    title = record.get("name") or title
                message = record.get("message") or {}
                if not title and (kind == "user" or message.get("role") == "user"):
                    content = message.get("content") or ""
                    if isinstance(content, list):
                        content = " ".join(b.get("text", "") for b in content if isinstance(b, dict))
                    title = str(content)[:160]
                included = bool(timestamp) and (since is None or local_time(timestamp) >= since)
                for call_id, name, inferred in tool_calls(record, provider):
                    key = (call_id, name, inferred)
                    if call_id and key in seen_calls:
                        continue
                    seen_calls.add(key)
                    if included:
                        row["mcp_refs" if inferred else "tools"][name] += 1
                event = None
                if provider == "codex":
                    info = payload.get("info") or {}
                    if kind == "token_usage_record":
                        modern = True
                        response_id = payload.get("response_id")
                        if response_id and response_id in seen_usage:
                            continue
                        if response_id:
                            seen_usage.add(response_id)
                        usage = payload.get("usage") or {}
                        cumulative = payload.get("thread_token_usage") or {}
                        previous = cumulative or previous
                    elif payload.get("type") == "token_count" and not modern:
                        cumulative = info.get("total_token_usage") or {}
                        if cumulative:
                            usage = {key: counter_delta(value, positive_int(previous.get(key)))
                                     for key, value in cumulative.items()}
                            previous = cumulative
                        else:
                            usage = info.get("last_token_usage") or {}
                    else:
                        continue
                    if usage:
                        event = usage_event(model, timestamp, provider=provider,
                            input_tokens=usage.get("input_tokens"), output_tokens=usage.get("output_tokens"),
                            cache_read=usage.get("cached_input_tokens"),
                            cache_write=usage.get("cache_write_input_tokens"),
                            total=usage.get("total_tokens", positive_int(usage.get("input_tokens"))
                                        + positive_int(usage.get("output_tokens"))))
                elif (kind == "assistant" or message.get("role") == "assistant") and message.get("usage"):
                    usage = message["usage"]
                    model = message.get("model") or "unknown"
                    if model == "<synthetic>":
                        continue
                    usage_id = (message.get("id") or record.get("id"), record.get("requestId"))
                    if usage_id[0] and usage_id in seen_usage:
                        continue
                    seen_usage.add(usage_id)
                    pi = provider == "pi"
                    event = usage_event(model, timestamp, provider=provider,
                        input_tokens=usage.get("input" if pi else "input_tokens"),
                        output_tokens=usage.get("output" if pi else "output_tokens"),
                        cache_read=usage.get("cacheRead" if pi else "cache_read_input_tokens"),
                        cache_write=usage.get("cacheWrite" if pi else "cache_creation_input_tokens"),
                        total=usage.get("totalTokens") if pi else None)
                if included and event and event["tokens"]:
                    for field in ("tokens", *TOKEN_FIELDS):
                        row[field] += event[field]
                    row["events"] += 1
                    row["models"].add(model)
            key = (provider, session_id)
            if key in seen_sessions:
                continue
            seen_sessions.add(key)
            meta = metadata.get(session_id, {}) if provider == "codex" else {}
            row.update(id=session_id, title=meta.get("title") or title or path.stem,
                       started=meta.get("started") or started)
            if row["tokens"] or row["tools"] or row["mcp_refs"]:
                yield row

    snapshots, sessions = {}, {}
    for record in jsonl_records((ANTIGRAVITY_USAGE_LOG,)):
        payload = record.get("payload") or record
        session_id = payload.get("conversation_id") or payload.get("session_id") or payload.get("transcript_path")
        timestamp = record.get("captured_at") or payload.get("created_at")
        if not session_id or not timestamp:
            continue
        context = payload.get("context_window") or {}
        totals = (positive_int(context.get("total_input_tokens")), positive_int(context.get("total_output_tokens")))
        delta_input, delta_output = map(counter_delta, totals, snapshots.get(session_id, (0, 0)))
        snapshots[session_id] = totals
        row = sessions.setdefault(session_id, {
            "id": session_id, "title": session_id, "started": local_time(timestamp),
            "provider": "antigravity", "tokens": 0, **dict.fromkeys(TOKEN_FIELDS, 0),
            "events": 0, "tools": Counter(), "mcp_refs": Counter(), "models": set(),
        })
        if since and local_time(timestamp) < since:
            continue
        model_info = payload.get("model") or {}
        row["models"].add(model_info.get("id") or model_info.get("display_name") or "unknown")
        row["input_tokens"] += delta_input
        row["output_tokens"] += delta_output
        row["tokens"] += delta_input + delta_output
        row["events"] += bool(delta_input or delta_output)
    yield from (row for row in sessions.values() if row["tokens"])


def print_deep_bd(period: str, sort: str, title: str, tool: str, mcp: bool,
                  limit: int | None = None, started_after: datetime | None = None) -> None:
    rows = list(scan_deep_sessions(period))
    rows = [r for r in rows if title.casefold() in r["title"].casefold()
            and (started_after is None or r["started"] is not None and r["started"] >= started_after)
            and (not tool or any(tool.casefold() in name.casefold()
                                 for name in r["tools"] | r["mcp_refs"]))
            and (not mcp or r["mcp_refs"] or any("mcp__" in name for name in r["tools"]))]
    def sort_key(row):
        if sort == "title":
            return row["title"].casefold()
        if sort == "start":
            return row["started"] or datetime.min.replace(tzinfo=LOCAL_TZ)
        if sort == "tools":
            return sum(row["tools"].values())
        if sort == "mcp":
            return sum(n for name, n in row["tools"].items() if "mcp__" in name) + sum(row["mcp_refs"].values())
        return row["tokens"]
    rows.sort(key=sort_key, reverse=sort != "title")
    rows = rows[:limit]
    print(f"tokscale · deep session breakdown · {period} · {LOCAL_TZ}")
    print("Tokens and calls are within the selected period; start is the original session start.")
    print("MCP refs are static references inside exec code, not verified executions.")
    print("Per-tool token costs are not recorded. Antigravity start means first counter observation.")
    for row in rows:
        clean_title = " ".join(row["title"].split())
        clean_title = "".join(c for c in clean_title if c.isprintable())
        if len(clean_title) > 160:
            clean_title = clean_title[:157] + "..."
        start = row["started"].strftime("%Y-%m-%d %H:%M:%S %Z") if row["started"] else "unknown"
        print(f"\n{clean_title}\n  {row['provider']} | {row['id']} | start {start}")
        print(f"  tokens {row['tokens']:,} | input {row['input_tokens']:,} | output {row['output_tokens']:,}"
              f" | cache read {row['cache_read_tokens']:,} | cache write {row['cache_write_tokens']:,}")
        print(f"  models: {', '.join(sorted(row['models'])) or 'unknown'}")
        mcp_calls = sum(n for name, n in row["tools"].items() if "mcp__" in name)
        print(f"  tool calls {sum(row['tools'].values())} | MCP calls {mcp_calls}: " +
              (", ".join(f"{name}={n}" for name, n in row["tools"].most_common()) or "none recorded"))
        if row["mcp_refs"]:
            print("  MCP refs in exec: " + ", ".join(f"{name}={n}" for name, n in row["mcp_refs"].most_common()))
    print(f"\n{len(rows)} sessions | {sum(r['tokens'] for r in rows):,} tokens")
    print("Codex cache read is included in input and total.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Report local AI CLI token usage")
    commands = parser.add_subparsers(dest="command", required=True)
    models = commands.add_parser("models", help="show usage grouped by model")
    models.add_argument("--breakdown", action="store_true", help="show token categories")
    models.add_argument("--deep-bd", action="store_true", help="show session tokens, titles, start times and tool/MCP calls")
    models.add_argument("--sort", choices=("tokens", "title", "start", "mcp", "tools"), default="tokens")
    models.add_argument("--title", default="", help="filter deep sessions by title substring")
    models.add_argument("--tool", default="", help="filter deep sessions by tool name substring")
    models.add_argument("--mcp", action="store_true", help="only deep sessions with MCP calls or references")
    models.add_argument("--limit", type=int, help="maximum deep sessions to show")
    models.add_argument("--started-after", help="only sessions started on/after ISO date or datetime; local timezone by default")
    periods = models.add_mutually_exclusive_group()
    for period in ("daily", "weekly", "monthly", "all"):
        periods.add_argument(f"--{period}", dest="period", action="store_const", const=period)
    periods.add_argument("--daily3", action="store_true", help="show today's top three models")
    argv = sys.argv[1:]
    if argv and argv[0].startswith("--") and "--deep-bd" in argv:
        argv.insert(0, "models")
    args = parser.parse_args(argv)
    if args.limit is not None and args.limit < 1:
        parser.error("--limit must be positive")
    if not args.deep_bd and (args.title or args.tool or args.mcp or args.limit is not None or args.sort != "tokens" or args.started_after):
        parser.error("session filters and sorting require --deep-bd")
    started_after = None
    if args.started_after:
        try:
            started_after = datetime.fromisoformat(args.started_after.replace("Z", "+00:00"))
        except ValueError:
            parser.error("--started-after must be an ISO date or datetime")
        if started_after.tzinfo is None:
            started_after = started_after.replace(tzinfo=LOCAL_TZ)
    period, limit = ("daily", 3) if args.daily3 else (args.period or "all", None)
    if args.deep_bd:
        print_deep_bd(period, args.sort, args.title, args.tool, args.mcp, args.limit or limit, started_after)
        return
    print_models(args.breakdown, period, limit)


if __name__ == "__main__":
    try:
        main()
    except BrokenPipeError:
        pass
